...
La web, Supabase, Netlify y el número de WhatsApp siguen siendo los existentes. El producto agéntico queda intacto y dormido mientras el webhook apunte a `waitlist-agent`.

## Flujo

1. La persona abre `/early-access`, verifica su teléfono con el `app-auth` existente y acepta explícitamente guardar la conversación y recibir WhatsApp.
2. `waitlist-start` verifica el JWT y que el teléfono pertenece a ese usuario.
3. Se envían exactamente dos plantillas aprobadas, una vez cada una:
...
3. Configurar las variables `WAITLIST_*` documentadas en `.env.membership.example`, además de las credenciales existentes de Supabase, OpenAI y el proveedor de WhatsApp.
