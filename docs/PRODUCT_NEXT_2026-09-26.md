...
La integración ya está publicada en **staging privado**, con Supabase independiente `njqbovsmoowkhhsqmitn` y migraciones `001`–`024`. Netlify `6ab7d0df552dead835399f9e` contiene exactamente cuatro funciones desde `5cab6d312f505b7395456b9d2864be69c978bd48`. Sus SHA256 remotos coinciden con los ZIP, la página y las funciones exigen contraseña, y no existen credenciales de transporte ni cron. El verificador inicial no contemplaba la respuesta de Netlify como objeto; se corrigió y verificó el mismo deploy en lectura, conservando el informe inicial, sin volver a publicar.

El smoke real de infraestructura pasa **4/4 y limpieza**: rechazo sin JWT, dos cuentas sintéticas sin envío, aislamiento de cuenta/instalación y restricciones RLS/RPC. No consume modelo y no cubre conversación completa, memoria entre proveedores ni recibos físicos. El harness integrado previo al deploy pasa **61/61** con scope; el replay limpio de 925 turnos pasa siete dimensiones deterministas, con cero llamadas de modelo y todos los textos aún sin verificación independiente. El [índice de evidencias](PRODUCT_NEXT_EVIDENCE.json) conserva alcance, paths y hashes de los resultados originales. Producción permanece sin cambios de esta tarea.
