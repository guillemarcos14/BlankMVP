...
function buildInsight(payload, memory = {}) {
  ...
  if ((weekly.health_signal_coverage_percent || 0) > 0 && weekly.health_signal_coverage_percent < 45) {
    recommendations.push("Sync your wearable daily so Blankmind can separate recovery dips from normal screen urges.");
  }
  ...
  if (appSequence) {
    ...
    if (/high|medium/i.test(sequenceRisk)) {
      recommendations.push("Start the selected-distractions block before the second quick check becomes a scroll session.");
    }
  }
  ...
  if (memory.ignoredActions?.includes("recovery_mode") || memory.ignoredActions?.includes("preventive_block")) {
    recommendations.push("Try a shorter selected-distractions block instead of repeating the same intensity.");
  }

  if (weekly.selection_count < 1) {
...
function normalizeBehaviorForecast(candidate, fallback = {}) {
  ...
  return {
    ...
    confidence: Math.min(100, Math.max(20, Math.round(cleanNumber(source.confidence) || fallback.confidence || 20))),
    reasons: (reasons.length ? reasons : fallback.reasons || ["Blankmind is learning your baseline."]).slice(0, 4),
...
function normalizeExperiment(candidate, fallback = {}) {
  ...
  return {
    ...
    variant: cleanText(source.variant, 60) || fallback.variant || "same_window_selected_distractions",
