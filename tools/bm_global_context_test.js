...
const legacyRevisionGuard = fs.readFileSync(path.join(root, "supabase/migrations/020_bm_legacy_context_snapshots.sql"), "utf8");
...
assert.match(legacyRevisionGuard, /excluded\.client_revision > bm_legacy_context_snapshots\.client_revision/i);
assert.match(legacyRevisionGuard, /upsert_bm_legacy_user_context/i);
...
assert.match(persistenceSource, /bm_legacy_context_snapshots/);
assert.match(persistenceSource, /rpc\/upsert_bm_legacy_user_context/);
...
assert.match(assistantChannelSource, /bm_identity_not_found/);
assert.match(assistantChannelSource, /enrichAssistantContext\(normalizedContext, connectCode\)/);
